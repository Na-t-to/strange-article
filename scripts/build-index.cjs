...
function todayBlock() {
  ...
  return `<p class="entry-label">追加日 ${esc(article.publishedAt)}</p><h2><a href="articles/${esc(article.slug)}.html">${esc(article.title)}</a></h2><p class="original-title">${esc(article.originalTitle)}</p><p class="entry-excerpt">${esc(article.excerpt)}</p><div class="entry-meta"><span>${article.tags.map((tag) => `<span>${esc(tag)}</span>`).join('')}</span><span>${esc(article.authors.join(' / '))} · ${esc(article.year)} · 読了 ${esc(article.readingMinutes)}分</span><span class="row-actions"><button type="button" data-read="${esc(article.slug)}" aria-pressed="false">✓ <span data-label>未読</span></button><button type="button" data-favorite="${esc(article.slug)}" aria-pressed="false">♡ <span data-label>保存</span></button></span></div>`;
}
