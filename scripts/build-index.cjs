...
function tableBlock() {
  ...
  const body = rows.map((article) => { const author = article.authors.length > 2 ? `${article.authors[0]} ほか${article.authors.length - 1}名` : article.authors.join(' / '); return `<tr data-slug="${esc(article.slug)}"><td class="date-cell">${esc(article.publishedAt)}</td><td class="title-cell"><a href="articles/${esc(article.slug)}.html">${esc(article.title)}</a><span class="original-title">${esc(article.originalTitle)}</span><div class="row-actions"><button type="button" data-read="${esc(article.slug)}" aria-pressed="false">✓ <span data-label>未読</span></button><button type="button" data-favorite="${esc(article.slug)}" aria-pressed="false">♡ <span data-label>保存</span></button></div></td><td class="tag-cell">${article.tags.map((tag) => `<span>${esc(tag)}</span>`).join('')}</td><td class="source-cell">${esc(author)}<br><span>${esc(article.year)}</span></td><td class="length-cell">${esc(article.readingMinutes)}分</td></tr>`; }).join('');
